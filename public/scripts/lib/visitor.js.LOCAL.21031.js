define(['jquery','utilities','globals','accounting','ezstorage'], function($,utilities,globals,accounting){
	...
	visitor.prototype.cartquerystring = function(url){
		if(this.shoppingCart)
			this.shoppingCart.forEach(function(product){
				url += (url.length>0?"&":"") + "pid[]=" + encodeURIComponent(product.packageId || "") + "&prid[]=" + encodeURIComponent(product.productId||"") + "&qty[]=" + encodeURIComponent(product.qty||1);
			});
	...
	function campaignInfo(){
		// this.campaignId;= utilities.querystring('caid') || '';
		this.campaignCode;// = utilities.querystring('cacode') || '';
		...
		for (var prop in this) {
			if (this.hasOwnProperty(prop)) {
				if(typeof this[prop].value == "string"){
					...
					if(prop=='valid') ret['valid'] = this[prop];
				}
	...
	visitor.prototype.copy = function(from,to){
        ...
        for (var prop in from) {
           if (from.hasOwnProperty(prop)) {
              if(to[prop].set) to[prop].set(from[prop].value);
